...
% tscore=ones(length(gridx),length(gridy));
fullscore=zeros(h,w);
...
%             mean_f=mean2;
%             tscore(ii,jj)=0;
            fullscore(yy:yy+patch_size-1, xx:xx+patch_size-1) = fullscore(yy:yy+patch_size-1, xx:xx+patch_size-1) + 1;
        end
